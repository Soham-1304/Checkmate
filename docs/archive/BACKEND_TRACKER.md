...
| `GET /api/v1/repository/search` (brand, manufacturer, commodity, barcode, date, officer, violation) + CSV/Excel export | 🟢 DONE (B7 live, CSV) | `repository.py` — flat rows, same-filter search+export, officer auto-scoped |
| `GET /api/v1/audit-events?entity_type=&entity_id=` | 🟢 DONE (B7 live) | `audit.py` — `can_view_audit`, entity/action filters, chronological |
...
- 2026-09-09: B7 DONE — repository search + CSV export + audit query. `repository.py`: `GET /repository/search` (brand/manufacturer/commodity ilike, barcode exact, officer_id, result, violation=FAIL, from/to dates, limit/offset conv, `{items,total}`) + `GET /repository/export` (same filters, streamed CSV ≤5000 rows); officer auto-scoped to own, `can_view_own_reports` gate. `audit.py`: `GET /audit-events` (entity_type/entity_id/action, chronological, `can_view_audit` → officer 403). Migration `b7_search_indexes` (4 indexes, applied live). E2E live on seeded FAIL: all facets + CSV columns + full trail (CREATED→EVALUATED→REPORT_GENERATED) + scoping → cleaned (both buckets + rows).
- 2026-09-09: B8 DONE — First Schedule MPE + Second Schedule pack-size validators (tables verbatim from user-supplied `RULES.md`, cross-checked vs Gazette OCR). Both are sub-checks under Rule6(1)(c) (no version surgery): MPE deficiency-only with % rounding rule + by-number 2%, no-physical = not-verifiable note; pack-size via 19-entry schedule map (BISCUITS/TEA/ATTA/EDIBLE_OIL/SOAP→toilet/COFFEE mapped, rest neutral), disclaimer cures FAIL. Findings carry `Rule6(1)(c); First Schedule Table-I` / `Rule 5 / Second Schedule` refs; numeric trace in `eval_detail.detail`. Live E2E: 480/500 FAIL → 490/500 PASS; 63g biscuit FAIL → +disclaimer PASS; SNACKS neutral PASS → cleaned (both buckets + rows). RULEBOOK_v1.md gaps flipped.
- 2026-09-09: DEPLOY AUTH FIX — fresh container resolved **bcrypt 5.0.0** (unbounded `passlib[bcrypt]`) → `ValueError: password cannot be longer than 72 bytes` on login (LOCAL was 4.3.0, worked). Pinned `bcrypt==4.3.0` in `requirements.txt`; Dockerfile smoke layer now asserts the auth roundtrip at BUILD time (`import app.main` + hash/verify) so a future dep bump fails the build, not the demo; `.dockerignore` added (was baking `backend/.env` → secrets + missing SECRET_KEY/CORS defaults); `alembic/env.py` guarded `os.path.exists(.env)` for container/Render. `analyze-auto` now also auto-renders the report PDF (non-fatal, returns `report_url`) so upload→OCR→report is genuinely one call. Scanner E2E verified against live backend: login → create → upload → analyze-auto → evaluate → result (REVIEW, findings, PDF) → test rows cleaned (repo = 3 demo inspections: oats/lays/amul).
- 2026-09-09 (evening): DEMO WORLD SEED v1 — `db/seeds/seed_demo_world.py` (idempotent, deterministic UUIDs, `--reset` flag). Adds 8 real brands (Parle-G/Britannia/Amul/Haldiram's/Tata/Maggi/Aashirvaad/Saffola) × 8 entities (plausible GSTINs) × 12 commodities (real EAN-13) + 2 extra officers (Priya Sharma DOCA-OFF-203, Rahul Verma DOCA-OFF-204) + 6 assignments + **15 inspections** across 10 states. Declarations fabricated per canonical key (12), then verdicts/findings produced by the REAL `run_compliance_evaluation` + auto-report via `render_inspection_report` (PDF fails locally — missing libgobject; OK in container). Evidence = 3 real photos uploaded to `Bluetick_Image_Store::demo/<insp>/`. Engineered outcome: 6 PASS / 5 FAIL / 4 REVIEW (font-0.7mm, MPE 230g-vs-250g, 4.9kg atta schedule, missing mfg address, low-confidence REVIEWs). Verified `/dashboard/admin?days=60` shows 16 active days, 3 officers, 10 states, 12 sectors, ai_quality {228 decls, 35 low-conf, 2 overrides}. Run: `python3 -m app.db.seeds.seed_demo_world [--reset]` (needs DYLD_FALLBACK_LIBRARY_PATH for local). Design spec: `docs/superpowers/specs/2026-09-09-demo-world-seed-and-live-ui-design.md`.
