...
export const AIDecisionCarousel: React.FC<AIDecisionCarouselProps> = ({ onReviewItem }) => {
  const { data: liveAIItems, live } = useLiveAIDecisions([]);
  ...
  return (
    <div className="bg-white rounded-3xl p-6 border border-slate-200/70 shadow-xs space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {liveAIItems.length === 0
                ? 'All inspections are up to date. No pending decisions.'
                : `${liveAIItems.length} inspection${liveAIItems.length !== 1 ? 's are' : ' is'} ready for your review.`}
            </p>
          </div>
        </div>
      ...
      {/* Grid of AI Analysis Cards or Empty State */}
      {liveAIItems.length === 0 ? (
        <div className="py-10 text-center space-y-2 bg-[#f8faf9] rounded-2xl border border-dashed border-slate-200">
          <CheckCircle2 className="w-8 h-8 text-[#017374] mx-auto opacity-70" />
          <div className="text-xs font-semibold text-slate-700">No inspections pending review</div>
          <p className="text-[11px] text-slate-400 max-w-sm mx-auto">
            When officers submit field inspections, automated RapidOCR and Legal Metrology rule assessments will surface here for clearance or violation issuance.
          </p>
        </div>
      ) : (
